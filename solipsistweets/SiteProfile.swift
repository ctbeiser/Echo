...
struct XSiteProfile: SiteProfile {
    ...
    func mapEchoDotAppToHTTPS(_ url: URL) -> URL? {
        Coordinator.mapEchoDotAppToHTTPS(url: url)
    }
...
struct RedditSiteProfile: SiteProfile {
    ...
    func mapEchoDotAppToHTTPS(_ url: URL) -> URL? {
        // Keep same behavior; if you don’t want echodotapp:// in Orion, return nil
        Coordinator.mapEchoDotAppToHTTPS(url: url)
    }
    var contentBlockerIdentifier: String { "com.orion.ContentBlocker.rules.v17" }
    var contentBlockerRulesJSON: String {
        // Only on the homepage (path exactly "/") for reddit.com and www.reddit.com
        // Hide #main-content and .masthead
        """
        [
          {
            "trigger": {
